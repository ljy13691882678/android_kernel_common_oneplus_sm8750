...
static int c_show(struct seq_file *m, void *v)
{
	...
	for_each_online_cpu(i) {
		if (i == 0) {
			seq_printf(m, "Processor\t: AArch64 Processor rev 0 (aarch64)\n");
			seq_printf(m, "Features\t: fp asimd evtstrm aes pmull sha1 sha2 crc32 atomics fphp asimdhp\n");
			seq_printf(m, "CPU implementer\t: 0x48\n");
			seq_printf(m, "CPU architecture: 8\n");
			seq_printf(m, "CPU variant\t: 0x1\n");
			seq_printf(m, "CPU part\t: 0xd0e\n");
		...
		if (i == 0) {
			...
			seq_printf(m, "CPU part\t: 0xd0f\n");
		} else {
			seq_printf(m, "CPU variant\t: 0x3\n");
			seq_printf(m, "CPU part\t: 0xd10\n");
		}
		seq_printf(m, "CPU revision\t: 0\n\n");
	}
